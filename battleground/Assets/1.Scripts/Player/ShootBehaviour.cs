...
/// 발사 키 입력받아서 애니메이션 재생, 이펙트 생성. 충돌체크 기능
/// UI 관련해서 십자선 표시 기능
/// 발사 속도 조정
/// 캐릭터 상체를 IK를 이용해서 조준 시점에 맞춰 회전.
/// 벽이나 충돌체 총알이 피격되었을 경우 피탄이펙트를 생성
/// 인벤토리 역할. 무기를 소지하고 있는지 확인용.
/// 재정전과 무기 교체 기능까지 포함하고 있음.
/// </summary>
public class ShootBehaviour : GenericBehaviour
{
    ...
    private Transform hips, spine, chest, righHand, leftArm;
    ...
    private void Start()
    {
        ...
        righHand = behaviourController.GetAnimator.GetBoneTransform(HumanBodyBones.RightHand);
        ...
        distToHand = (righHand.position - neck.position).magnitude * 1.5f;
    }

    //발사 비주얼 담당
    private void DrawShoot(GameObject weapon, Vector3 destination, Vector3 targetNormal, Transform parent, bool placeSparks = true, bool placBulletHole = true)
    {
        ...
        GameObject instaceShot = EffectManager.Instance.EffectOneShot((int)EffectList.tracer , origin);
        instaceShot.SetActive(true);
        instaceShot.transform.rotation = Quaternion.LookRotation(destination - origin);
        instaceShot.transform.parent = shot.transform.parent;
        if (placeSparks)
        {
    ...
    {
        newWeapon.gameObject.transform.SetParent(righHand);
